...
router.route("/reply").post(verifyToken, adminController.reply);

router.route("/:userID").delete(verifyToken, adminController.deleteUser);
...
router
  ...
  .post(
    verifyToken,
    uploadFileMW().single("file"),
    adminController.addListTeacher
  );
